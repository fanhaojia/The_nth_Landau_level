...
if __name__ == "__main__":
    ...
    j4=2
    j5=1
    y1= 0.000260416666667*x**(2*j1)*np.exp(-x**2/2)*hlcjh(0,j1+1,x**2/2)**2
    y2= 0.0130208333333*x**(2*j2)*np.exp(-x**2/2)*hlcjh(-1,j2+1,x**2/2)**2
    y3= 0.208333333333*x**(2*j3)*np.exp(-x**2/2)*hlcjh(-2,j3+1,x**2/2)**2
    y4= 1.25*x**(2*j4)*np.exp(-x**2/2)*hlcjh(-3,j4+1,x**2/2)**2
    y5= 2.5*x**(2*j5)*np.exp(-x**2/2)*hlcjh(-4,j5+1,x**2/2)**2
    y=y1+y2+y3+y4+y5
    while i>=0:
        a=0 #积分上限
        b=50.0 #积分下限
